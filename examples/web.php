...
Route::get('/api/database-example', function () {
    ...
    // Example of database operation tracing with Laravel Query Builder
    ...
    file_put_contents('/tmp/debug.log', "About to call traceDatabase\n", FILE_APPEND);
    if (isset($GLOBALS['simple_tracer'])) {
        $GLOBALS['simple_tracer']->traceDatabase($query, 'laravel', null, $duration, 1);
    } else {
        file_put_contents('/tmp/debug.log', "ERROR: simple_tracer not found!\n", FILE_APPEND);
    }
    file_put_contents('/tmp/debug.log', "After traceDatabase call\n", FILE_APPEND);
    ...
    return response()->json([
        'message' => 'Database operation with tracing',
        'result' => $result[0]->example
    ]);
});
